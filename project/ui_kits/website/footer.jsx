const { Button, Icon } = window.DrRomaDubeyDesignSystem_317be8;

function Footer({ onConnect }) {
  const cols = [
    ...
    <footer style={{ background: 'var(--sand-900)', color: 'var(--sand-200)' }}>
      <div className="container" style={{ paddingBlock: 'var(--space-16)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1.2fr', gap: 40 }}>
          <div>
            <div style={{ fontSize: 24, fontWeight: 800, letterSpacing: '-0.03em', color: '#fff' }}>Dr. Roma <span style={{ color: 'var(--clay-400)' }}>Dubey</span></div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
              <span style={{ display: 'flex', gap: 10, alignItems: 'flex-start', fontSize: 'var(--text-sm)', color: 'var(--sand-200)' }}><Icon name="map-pin" size={18} /> Maitri Mahila Clinic, Kaland Chungi, Sardhana · Aryavart Hospital, NH-58, Daurala, Meerut</span>
              <span style={{ display: 'flex', gap: 10, alignItems: 'center', fontSize: 'var(--text-sm)', color: 'var(--sand-200)' }}><Icon name="clock" size={18} /> Mon–Sat · 10:00 AM – 6:00 PM</span>
            </div>
