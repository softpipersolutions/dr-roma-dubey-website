const { Button, Badge, Icon } = window.DrRomaDubeyDesignSystem_317be8;

function Hero({ onConnect }) {
  const P = window.RD_PHOTOS;
  return (
    <section id="home" style={{ position: 'relative', overflow: 'hidden' }}>
      <img src={window.RD_BLOBS.blush} alt="" aria-hidden="true" style={{ position: 'absolute', top: -160, right: -120, width: 560, opacity: 0.7, pointerEvents: 'none' }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.05fr 0.95fr', gap: 56, alignItems: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
            ...
            <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginTop: 4 }}>
              <Button variant="whatsapp" size="lg" leftIcon="message" onClick={onConnect}>Connect on WhatsApp</Button>
              <Button variant="outline" size="lg" as="a" href="#services" rightIcon="arrow-right">Explore services</Button>
            </div>
            <div style={{ display: 'flex', gap: 28, marginTop: 14 }}>
              {[['8+', 'Years of care'], ['5000+', 'Women helped'], ['2', 'Clinic locations']].map(([n, l]) => (
                <div key={l}>
          ...
          <div style={{ position: 'relative' }}>
            <img src={window.RD_BLOBS.clay} alt="" aria-hidden="true" style={{ position: 'absolute', bottom: -50, left: -60, width: 340, opacity: 0.55, pointerEvents: 'none' }} />
            <div style={{ position: 'relative', borderRadius: 'var(--radius-2xl)', overflow: 'hidden', boxShadow: 'var(--shadow-xl)', aspectRatio: '4/5' }}>
              <img src={P.doctor} alt="Dr. Roma Dubey" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </div>
            <div style={{ position: 'absolute', bottom: 22, left: -26, background: 'var(--surface-card)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-lg)', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ width: 42, height: 42, borderRadius: '50%', background: 'var(--sage-100)', color: 'var(--sage-700)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon name="shield-check" size={22} /></span>
              <div>
