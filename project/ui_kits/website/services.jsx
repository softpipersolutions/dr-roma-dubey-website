const { ServiceCard, Button } = window.DrRomaDubeyDesignSystem_317be8;
    ...
    <section id="services" className="section">
      <div className="container">
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 24, marginBottom: 40, flexWrap: 'wrap' }}>
          ...
          <Button variant="outline" as="a" href="#services" rightIcon="arrow-right">View all services</Button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
          {SERVICES.map((s) => (
            <div key={s.title} className="reveal"><ServiceCard {...s} href="#" /></div>
          ))}
        </div>
      </div>
    </section>
