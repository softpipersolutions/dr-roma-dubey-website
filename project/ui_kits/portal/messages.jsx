const { Avatar, Icon, Badge } = window.DrRomaDubeyDesignSystem_317be8;

const THREADS = [
  { id: 'roma', name: 'Dr. Roma Dubey', preview: 'Your ultrasound looks great — let\'s review it on Tuesday.', time: '9:24 AM', unread: 1, initials: 'RD', accent: true },
...
function Messages() {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 0, background: 'var(--surface-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)', height: 520 }}>
        {/* threads */}
        <div style={{ borderRight: '1px solid var(--border-subtle)', overflowY: 'auto' }}>
          {THREADS.map((t) => {
            const on = active === t.id;
            return (
              <button key={t.id} onClick={() => setActive(t.id)} style={{ width: '100%', display: 'flex', gap: 12, padding: '16px 18px', border: 'none', borderBottom: '1px solid var(--border-subtle)', background: on ? 'var(--clay-50)' : 'transparent', cursor: 'pointer', textAlign: 'left', font: 'inherit' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px', borderBottom: '1px solid var(--border-subtle)' }}>
            ...
            <div><div style={{ fontWeight: 700, color: 'var(--text-strong)', fontSize: 'var(--text-sm)' }}>Dr. Roma Dubey</div><div style={{ fontSize: 11, color: 'var(--success-text)' }}>online</div></div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderTop: '1px solid var(--border-subtle)' }}>
            ...
            <span style={{ width: 42, height: 42, borderRadius: '50%', background: 'var(--clay-600)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Icon name="message" size={19} /></span>
          </div>
        </div>
      </div>
    </div>
