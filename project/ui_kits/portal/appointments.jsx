const { Tabs, Badge, Button, IconButton, Icon } = window.DrRomaDubeyDesignSystem_317be8;
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 18, padding: '16px 4px', borderBottom: '1px solid var(--border-subtle)' }}>
      ...
      {past
        ...
        : <div style={{ display: 'flex', gap: 6 }}><Button variant="outline" size="sm">Reschedule</Button><IconButton icon="video" label="Join" variant="soft" size="sm" /></div>}
    </div>
  );
}

function Appointments({ onBook }) {
  return (
    <div>
      ...
      <Tabs defaultValue="up" items={[
        { value: 'up', label: 'Upcoming', icon: 'calendar', content: (
          <div>{UPCOMING.map((a) => <AptRow key={a.title} a={a} />)}</div>
        ) },
        { value: 'past', label: 'Past visits', icon: 'clock', content: (
          <div>{PAST.map((a) => <AptRow key={a.title} a={a} past />)}</div>
        ) },
      ]} />
    </div>
  );
