  ...
  return (
    <div>
      ...
      <Card variant="elevated" padding="none">
        <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 0.8fr 0.9fr 0.7fr 0.5fr', padding: '14px 22px', fontSize: 'var(--text-xs)', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-subtle)', borderBottom: '1px solid var(--border-subtle)' }}>
          <span>Report</span><span>Type</span><span>Date</span><span>Status</span><span></span>
        </div>
        {REPORTS.map((r, i) => (
          <div key={r.name} style={{ display: 'grid', gridTemplateColumns: '1.6fr 0.8fr 0.9fr 0.7fr 0.5fr', alignItems: 'center', padding: '15px 22px', borderTop: i ? '1px solid var(--border-subtle)' : 'none' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 12, fontWeight: 600, color: 'var(--text-strong)' }}>
              <span style={{ width: 36, height: 36, borderRadius: 'var(--radius-sm)', background: 'var(--clay-50)', color: 'var(--clay-600)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Icon name="file-text" size={18} /></span>
              {r.name}
            </span>
            ...
            <span style={{ display: 'flex', justifyContent: 'flex-end', color: r.status === 'Ready' ? 'var(--clay-600)' : 'var(--sand-300)', cursor: r.status === 'Ready' ? 'pointer' : 'default' }}><Icon name="download" size={19} /></span>
          </div>
        ))}
      </Card>
    </div>
