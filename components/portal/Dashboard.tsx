...
import { Card, Badge, Button, Icon, type IconName } from '@/components/ds';
...
function Stat({ icon, tone, label, value, sub }: { icon: IconName; tone: string; label: string; value: string; sub: string }) {
  ...
  return (
    <Card variant="elevated" padding="md">
      <span style={{ display: 'inline-flex', width: 42, height: 42, borderRadius: 'var(--radius-md)', background: t[0], color: t[1], alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
...
export function Dashboard({ onBook }: { onBook: () => void }) {
  const reports: { name: string; date: string; status: 'ready' | 'pending'; tone: BadgeTone }[] = [
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      ...
      <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 'var(--radius-xl)', background: 'linear-gradient(120deg, var(--clay-600), var(--clay-500))', color: '#fff', padding: '28px 30px', boxShadow: 'var(--shadow-brand)' }}>
        ...
        <img src={BLOBS.cream} alt="" aria-hidden="true" style={{ position: 'absolute', top: -80, right: -40, width: 300, opacity: 0.25 }} />
        <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'rgba(255,255,255,0.2)', padding: '4px 12px', borderRadius: 'var(--radius-pill)', fontSize: 12.5, fontWeight: 600, marginBottom: 12 }}>
              <Icon name="clock" size={14} /> In 2 days
            </div>
      ...
      {/* stats */}
      <div className="portal-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
      ...
      {/* reports + tip */}
      <div className="portal-two" style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 16 }}>
          ...
          <div>
            {reports.map((r, i) => (
              <div key={r.name} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 22px', borderTop: i ? '1px solid var(--border-subtle)' : 'none' }}>
                <span style={{ width: 40, height: 40, borderRadius: 'var(--radius-sm)', background: 'var(--clay-50)', color: 'var(--clay-600)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <span style={{ color: r.status === 'ready' ? 'var(--clay-600)' : 'var(--sand-300)', display: 'flex', cursor: r.status === 'ready' ? 'pointer' : 'default' }}>
        ...
        <Card variant="cream" padding="lg">
          <span style={{ display: 'inline-flex', width: 42, height: 42, borderRadius: '50%', background: 'var(--surface-card)', color: 'var(--clay-600)', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
